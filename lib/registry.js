...
function validateManifest(id, raw, errors) {
  ...
  // Un slot que le core ne connait pas est une faute de frappe, pas une idee :
  // on refuse le plugin plutot que de le charger en widget que personne ne
  // peut selectionner.
  if (raw.defaultFor != null && raw.defaultFor !== "" && raw.defaultFor !== "link") {
    errors.push(`manifest.json : "defaultFor" (${str(raw.defaultFor, 40)}) inconnu, seul "link" existe`);
    return null;
  }
